...
namespace PC_05_SMKN_1_Bondowoso
{
    public partial class MasterDokter : Form
    {
        ...
        {
            string sql = "select * from admin where level = 2 and username like '%"+textBox1.Text+"%'";
            dataGridView1.DataSource = Command.getdata(sql);
            dataGridView1.Columns[2].Visible = false;
            dataGridView1.Columns[4].Visible = false;
        ...
        private void btnsimpan_Click(object sender, EventArgs e)
        {
            ...
            {
                string com = "insert into admin values('" + textBox2.Text + "', '" + textBox3.Text + "', 1, 2, '" + textBox5.Text + "')";
                try
                {
                    Command.exec(com);
                    loadgrid();
            ...
            {
                string com = "update admin set nama = '" + textBox5.Text + "', username = '" + textBox2.Text + "' where level = 2 and id = " + id;
                try
                {
                    Command.exec(com);
                    loadgrid();
